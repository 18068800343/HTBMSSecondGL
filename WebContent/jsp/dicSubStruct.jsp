                            ...
                            <div class="widget-body no-padding">
                                ...
                                <!-- 						<div class="widget-body-toolbar bg-color-white">
                                                            <form class="form-inline"   >
                                                                <div class="row col-sm-12 col-md-12 text-align-right">
                                                                <a class="btn btn-primary" onclick="newData()">增加</a>
                                                                </div>
                                                            </form>
                                                        </div> -->
    ...
    <form>
        <fieldset>
            <div class="form-group">
                <label>分部结构编号</label>
                <input class="form-control" id="distr_id" value="" placeholder="分部结构编号" type="text">
            </div>
            <div class="form-group">
                <label>分部结构名称</label>
                <input class="form-control" id="distr_name" value="" placeholder="分部结构名称" type="text">
            </div>
        </fieldset>
    </form>
</div>
...
<script type="text/javascript">
    ...
    //初始化表格
    var editDel = "<div class='text-align-center'><button class='edit btn btn-warning btn-xs'><span class='glyphicon glyphicon-pencil'></span></button>&nbsp;&nbsp;&nbsp;<button class='del btn btn-warning btn-xs'><span class='glyphicon glyphicon-trash'></span></button></div>";
    ...
    function initTable() {
        $.ajax({
            type: 'POST',
            url: '../DicSubStructServlet',
            dataType: 'json',
            data: {
                type: "initSubStruct"
            },
            error: function (msg) {
                errMessage("请求DicSubStructServlet失败");
            },
        ...
        $.SmartMessageBox({
            ...
            if (ButtonPressed === "确定") {
                $.ajax({
                    ...
                    dataType: 'json',
                    data: {
                        type: "deleteSubStruct",
                        distr_id: data.distr_id
                    },
                    error: function (msg) {
                        errMessage("请求DicSubStructServlet失败");
                    },
    ...
    $('#dt_basic').delegate('.edit', 'click', function () {
        var dom = $(this).parents('tr');
        var data = table.row(dom).data();
        $('#distr_id').val(data.distr_id);
        $('#distr_name').val(data.distr_name);
        $('#distr_id').prop('readonly', 'readonly');
        $('#dig').dialog({
            buttons: [
                {
                    ...
                    click: function () {
                        var info = {
                            distr_id: $('#distr_id').val(),
                            distr_name: $('#distr_name').val()
                        }
                        if (info.brg_type_id == "" || info.brg_type_name == "") {
                            errMessage("不可为空");
                        } else {
                            $.ajax({
                                type: 'POST',
