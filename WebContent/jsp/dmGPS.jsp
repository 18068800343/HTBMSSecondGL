	...
	<body class="">
		...
		<div id="main" role="main">
			...
			<div id="ribbon">
				...
				<!-- breadcrumb -->
				<ol class="breadcrumb">
					<li>实时监测</li><li>数据管理</li><li>GPS</li>
				</ol>
				<!-- end breadcrumb -->
				...
				if(data[i].state){
					var full=data[i];
					 if(full.is_download==1){
						 var n = full.data_file.slice(3,4);
						 var thunder=ThunderEncode("ftp://ht:ht123,@115.159.78.246/"+full.bridge_no+"_0"+full.data_file.slice(3,4)+"G/"+full.data_file);
						 link=link+thunder+"\r\n";
			    	  }else{
			    		  var thunder=ThunderEncode("ftp://ht:ht123,@115.159.109.101/"+full.bridge_no+"_0"+full.data_file.slice(3,4)+"G/"+full.data_file);
						  link=link+thunder+"\r\n";
			    	  }
				}
			}
			...
			for(var i=0;i<data.length;i++){
				if(data[i].state){
					var full=data[i];
					 if(full.is_download==1){
						 var n = full.data_file.slice(3,4);
						 var thunder="ftp:\/\/ht:ht123,@115.159.78.246/"+full.bridge_no+"G_0"+n+"/"+full.data_file;
						 //link.push(ThunderEncode(thunder));
						 link.push(thunder);
			    	  }else{
			    		 var thunder="ftp:\/\/ht:ht123,@115.159.109.101/"+full.bridge_no+"G_0"+n+"/"+full.data_file;
			    		  //link.push(ThunderEncode(thunder));
			    		  link.push(thunder);
			    	  }
					 console.log(full);
		...
		function isIE() { //ie?
						...
						"columns": [
							 ...
							 "columnDefs": [ 
									    ...
									    "render": function(data, type, full) {
									      ...
									      "render": function(data, type, full) {
									    	  if(full.is_download==1){
									    		  return "<a href='ftp://ht:ht123,@115.159.78.246/"+full.bridge_no+"_0"+full.data_file.slice(3,4)+"G/"+full.data_file+"' download='"+full.data_file+"'>下载</a>";
									    	  }else{
									    		  return "<a href='ftp://ht:ht123,@115.159.109.101/"+full.bridge_no+"_0"+full.data_file.slice(3,4)+"G/"+full.data_file+"' download='"+full.data_file+"'>下载</a>";
									    	  }
									    	  }
									    },
									    {
					...
					success : function(json) {   
						 if(json.success=="fail"){
							...
							default:
							 ...
							 $('.tree').append(buildTree((json.obj)[0]));	
							 appendTree((json.obj)[1]);
							 treeControl();
						 }
					}
				});
			...
			function appendTree(map){
				...
				for(var key in map){
					  ...
					  for(var i=0;i<map[key].length;i++){
						  console.log("属性"+map[key][i]);
						  if(flag){
							  $("#"+key).append("<ul hidden='hidden'></ul>")
						  }
							if(map[key][i].out_time!="1"){							
							  $("#"+key+" ul").append(" <li  data-dir='2017-11-19 01:04:55' id='"+map[key][i].id+"'><span class='tree-third' > "+map[key][i].brg_pass_no+" </span><i style='font-style:normal;color:#214e75'>---无数据接入("+formatEndTime(map[key][i].out_time)+")</i>");
							}
							if(map[key][i].out_time=="1"){	
							  $("#"+key+" ul").append(" <li  data-dir='2017-11-19 01:04:55' id='"+map[key][i].id+"'><span class='tree-third' > "+map[key][i].brg_pass_no+" </span><i style='font-style:normal;color:#214e75'></i>");
							}
						  flag=false;
				...
				for(var i=0;i<data.length;i++){
					...
					if(data[i].children.length>0){
						...
						if(data[i].bgco==undefined||data[i].bgco==""){
							li.append("<span class='tree-second'><i class='fa fa-lg fa-plus-circle'></i>"+data[i].name+"</span>");
						}else{
							
							if(data[i].bgco!="1"){
								li.append("<span class='tree-second'><i class='fa fa-lg fa-plus-circle'></i>"+data[i].name+"</span><i style='font-style:normal;color:#214e75'>---无数据接入("+formatEndTime(data[i].bgco)+")</i>");
							}else{
								li.append("<span class='tree-second'><i class='fa fa-lg fa-plus-circle'></i>"+data[i].name+"</span><i style='font-style:normal;color:#214e75'></i>");
							}
			...
			function treeControl(){
				...
				$('.tree-third').on('click',function(){
					...
					var info={};
					info.id = nameid;
					info.idd = $(treeSecondSelect).closest('li').attr('id');
					initTable(info);
				});
