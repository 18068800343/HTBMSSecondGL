            ...
            <div class="row">
                ...
                <article class="col-sm-12 col-md-12 col-lg-12">
                    ...
                    <div class="jarviswidget jarviswidget-color-darken" id="wid-id-1" data-widget-colorbutton="false"
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-sm-12 col-md-12 text-align-right">
                                            ...
                                            <div class="pull-left col-xs-3 pull-left"><input class="form-control"
                                                                                             ...
                                                                                             placeholder="搜索"
                                                                                             type="text"></div>
                                            <div id="eval_label" class="pull-left"></div>
                        ...
                        <%
                        } else if (oc.getMode().equals("bridge")) {
                        %>
                        <script>location.href = "checkBridge.jsp";</script>
                        <%
                            }
                        %>
                        <script>var info =
    ...
    <form class="col-xs-6">
        <fieldset>
            ...
            <div class="form-group">
                ...
                <!-- 		<input class="form-control" id="chk_weather" value="" placeholder="检查天气" type="text"> -->
...
<div id="eval" hidden="hidden">
    <form class="">
        <fieldset>
            <div class="form-group">
                <label>评定等级</label>
                <select class="form-control input-sm" id="eval_level">
                    <option>好</option>
                    <option>较好</option>
                    <option>差</option>
                    <option>较差</option>
            ...
            <div class="form-group">
                <label>检验员</label>
                <select class="form-control input-sm" id="inspection_person">
                </select>
            </div>
        </fieldset>
    </form>
...
// 		    var result =/\.[^\.]+/.exec(file_name);
    ...
    //判断状态，是否存在正在进行项目 和 项目下是否存在当前通道结构
    var bridgeChk = {
        prj_id: undefined,
        prj_desc: undefined,
        chk_type: undefined,
            ...
            success: function (json) {
                if (json.success == "fail") {
                    ...
                    if(info.prj_state==0){
                        ...
                        if (bridgeChk.eval_level != undefined) {
                            $('#eval_label').append("<h4><label class='label label-info'>评定等级：" + bridgeChk.eval_level + "</label></h4>");
                        }
                        if (bridgeChk.audit_state == '2') {
                            ...
                            editDel = "<div class='text-align-center'><button class='edit btn btn-warning btn-xs' disabled><span class='glyphicon glyphicon-pencil'></span></button>&nbsp;&nbsp;&nbsp;<button class='del btn btn-warning btn-xs' disabled><span class='glyphicon glyphicon-trash'></span></button></div>";
                        } else {
                            permision = true;
                        }
                        getSpan();
    ...
    $('#dt_basic').delegate('.add', 'click', function () {
        ...
        window.location.href = encodeURI("checkPassSpan.jsp");
    });
    ...
    function buildSpan() {
        ...
        for (var i = 0; i < data.length; i++) {
            $('#span_no').append("<option>" + data[i].span_no + "</option>");
        }
        $('#span_no').select2();
    }
            ...
            success: function (json) {
                if (json.success == "fail") {
                    ...
                    for (var i = 0; i < ss.length; i++) {
                        $('#recheck_person,#inspection_person').append("<option>" + ss[i] + "</option>");
                    }
                            ...
                            click: function () {
                                $.ajax({
                                    ...
                                    data: {
                                        type: "overCheck",
                                        chk_id: bridgeChk.chk_id,
                                        eval_level: $('#eval_level').val(),
                                        inspection_person: $('#inspection_person').val()
                                    },
                                    error: function (msg) {
                                        errMessage("请求CheckPassServlet失败");
                                    },
            ...
            $.ajax({
                ...
                success: function (json) {
                    if (json.success == "fail") {
                        ...
                        $('.widget-body-toolbar a').eq(2).removeClass('btn-success disabled').addClass('btn-primary').text('完成通道检查');
                    }
                }
            });
    ...
    function releaseMember() {//项目参与人
    	 if(info.prj_state==0){
    		  join = true;
    	        $('.widget-body-toolbar a').eq(0).removeClass('btn-warning disabled').addClass('btn-primary').text('照片');
    	        $('.widget-body-toolbar a').eq(1).removeClass('btn-warning disabled').addClass('btn-primary').text('选择跨');
    	        $('.widget-body-toolbar a').eq(2).removeClass('btn-warning disabled').addClass('btn-primary').text('完成通道检查');
