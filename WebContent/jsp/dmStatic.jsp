				...
				if(data[i].state){
					var full=data[i];
					 if(full.is_download==1){
						 var thunder=ThunderEncode("ftp://ht:ht123,@115.159.78.246/"+full.bridge_no+"J/"+full.data_file);
						 link=link+thunder+"\r\n";
			    	  }else{
			    		  var thunder=ThunderEncode("ftp://ht:ht123,@115.159.109.101/"+full.bridge_no+"J/"+full.data_file);
						  link=link+thunder+"\r\n";
			    	  }
				}
			}
		...
		function getLink2(){
			...
			for(var i=0;i<data.length;i++){
				if(data[i].state){
					var full=data[i];
					 if(full.is_download==1){
						 var thunder="ftp:\/\/ht:ht123,@115.159.78.246/"+full.bridge_no+"J/"+full.data_file;
						 //link.push(ThunderEncode(thunder));
						 link.push(thunder);
			    	  }else{
			    		  var thunder="ftp:\/\/ht:ht123,@115.159.109.101/"+full.bridge_no+"J/"+full.data_file;
			    		  //link.push(ThunderEncode(thunder));
			    		  link.push(thunder);
			    	  }
				}
						...
						"columns": [
							 ...
							 "columnDefs": [ 
									    ...
									    "render": function(data, type, full) {
									      ...
									      "render": function(data, type, full) {
									    	  if(full.is_download==1){
									    		  return "<a href='ftp://ht:ht123,@115.159.78.246/"+full.bridge_no+"J/"+full.data_file+"' download='"+full.data_file+"'>下载</a>";
									    	  }else{
									    		  return "<a href='ftp://ht:ht123,@115.159.109.101/"+full.bridge_no+"J/"+full.data_file+"' download='"+full.data_file+"'>下载</a>";
									    	  }
									    	  }
									    },
									    {
