    ...
    <form>
        <fieldset>
            <div class="form-group">
                <label>方向</label>
                <select class="form-control" id="culvert_direction">
                    <option value='无'>无</option>
                </select>
            </div>
            <div class="form-group">
                <label>跨号</label>
                <input class="form-control" id="culvert_span" value="" placeholder="跨号" type="text">
            </div>
            <div class="form-group">
                <label>涵洞类型</label>
                <select class="form-control select2" id="culvert_type" style="width:100%">
                </select>
            </div>
        </fieldset>
    </form>
...
<div id="copyMem" hidden="hidden">
    ...
    <div class="text-align-center">
        <button class="btn btn-default" onclick="copyAllMem()">复制全涵洞构件</button>&nbsp;&nbsp;&nbsp;&nbsp;
...
<script type="text/javascript">
    ...
    var session = <%=JSON.toJSONString(oc)%>
        ...
        $(document).ready(function () {
            ...
            /*	"sDom": //"<'dt-toolbar'<'col-xs-12 col-sm-6 hidden-xs'f><'col-sm-6 col-xs-12 hidden-xs'<'toolbar'>>r>"+
             ...
             $("#datatable_fixed_column thead th select").on( 'keyup change', function () {
            ...
            /*
             ...
             for(var a=0;a<len;a++){
             var ss=json.obj[a].highway_id;
             $("#tree-ul").append("<li class='parent_li' id='1"+ss+"'><span title='展开' id='"+ss+"' onclick='hs(this.id)' style='font-size:10px;font-family:微软雅黑' class='treess'><i id='i"+ss+"'class='fa fa-lg fa-plus-circle'>"+json.obj[a].highway_name+"</span></li>");
             }
             }
             }
             });
            ...
            /*
             ...
             var len=json.obj.length;
             for(var a=0;a<len;a++){
             $("#gjlx123").append("<option value='"+json.obj[a].component_name+"'>"+json.obj[a].component_name+"</option>");
             $("#ggjlx").append("<option value='"+json.obj[a].component_name+"'>"+json.obj[a].component_name+"</option>");
             }
            ...
            /*
             ...
             var len=json.obj.length;
             for(var a=0;a<len;a++){
             $("#bbjlx").append("<option value='"+json.obj[a].component_name+"'>"+json.obj[a].component_name+"</option>");
             $("#bbjlx123").append("<option value='"+json.obj[a].component_name+"'>"+json.obj[a].component_name+"</option>");
             }
    ...
    function initTreeRoot() {
        $.ajax({
            ...
            success: function (json) {
                if (json.error == 0) {
                    var culvert_id = json.success
                    $("#tree-ul").append("<li class='parent_li' id='culvert_root'><span title='展开' id='" + culvert_id + "' onclick='hs(this.id)' style='font-size:10px;font-family:微软雅黑' class='treess'><i id='i" + culvert_id + "'class='fa fa-lg fa-plus-circle'>" + culvert_id + "</span></li>");
                }
            }
        });
    }
    ...
    /**加载涵洞类型*/
    function loadCulvert_type() {
        var culvert_type = $("#culvert_type");
        $.ajax({
            type: 'POST',
            url: '../CulMbrServlet',
            dataType: 'json',
            ...
            success: function (json) {
                if (json.success == "success") {
                    ...
                    for (var i = 0; i < arr.length; i++) {
                        ...
                        $option.prop("value", arr[i].cul_type_id);
    ...
    function type_id2name(str) {
        ...
        for (var i = 0; i < arr_type.length; i++) {
            if (str == arr_type[i].cul_type_id) {
                name = arr_type[i].cul_type_name;
            }
        }
        return name;
    }
    ...
    function type_name2id(str) {
        var name;
        for (var i = 0; i < arr_type.length; i++) {
            if (str == arr_type[i].cul_type_name) {
                name = arr_type[i].cul_type_id;
            }
        }
        return name;
    }
    ...
    function addSpan() {
        ...
        $("#culvert_span").val("");
        //$("#culvert_type").val("").trigger("change");
        //$("#culvert_direction").val("").trigger("change");
            ...
            buttons: [
                {
                    ...
                    click: function () {
                        ...
                        $.ajax({
                            ...
                            dataType: 'json',
                            data: {
                                choice: "add_culvert_span",
                                //s_id:s_id,
                                culvert_direction: $("#culvert_direction").val(),
                                culvert_span: $("#culvert_span").val(),
                                culvert_type: $("#culvert_type").val()
                            },
                            error: function (msg) {
                                errMessage("添加跨号失败！");
                            },
                            success: function (json) {
                                if (json.success == "success") {
                                    //successMessage("添加成功！");
                                    var span = json.obj;
                                    var name = type_id2name(span.cul_type_id);
                                    var $li = $("<li></li>");
                                    $li.attr("data-id", span.s_id);
                                    $li.attr("data-type", span.cul_type_id);
            ...
            success: function (json) {
                ...
                if (json.success == "success") {
                    ...
                    for (var i = 0; i < len; i++) {
                        ...
                        $li.attr("data-type", json.obj[i].cul_type_name);
                        $li.attr("data-direction", json.obj[i].direction);
                        $li.attr("data-name", json.obj[i].span_no);
                        $li.append("<span class='kh col-lg-2 col-md-2 col-xs-2' onclick='choiceSpan(this)'>" + json.obj[i].span_no + "</span>&nbsp;--&nbsp;" + "<i style='font-style:normal;color:#214e75'>" + json.obj[i].cul_type_name + "</i>");
                        $li3.children("ul").append($li);
                    }
    ...
    function evalSpan() {
        $("#culvert_direction").val($lli.attr("data-direction"));
        $("#culvert_span").val($lli.attr("data-name"));
        $("#culvert_type").val(type_name2id($lli.attr("data-type"))).trigger("change");
                    ...
                    click: function () {
                        ...
                        $.ajax({
                            ...
                            data: {
                                choice: "editspan",
                                s_id: s_id,
                                span_no: $("#culvert_span").val(),
                                cul_type_id: $("#culvert_type").val(),
                                direction: $("#culvert_direction").val()
                            },
                            error: function (msg) {
                                errMessage("修改跨号失败！");
                            },
                            success: function (json) {
                                if (json.success == "success") {
                                    ...
                                    $lli.attr("data-type", span.cul_type_id);
    ...
    function edit(a) {
        if (hhhbs == null || hhhbs == undefined) {
            ...
            $('#opera').dialog("open");
            bjlx = $("#cttt tr:eq(" + a + ") td:eq(1)").text();
            gjlx = $("#cttt tr:eq(" + a + ") td:eq(2)").text();
            gjbh = $("#cttt tr:eq(" + a + ") td:eq(3)").text();
            gjms = $("#cttt tr:eq(" + a + ") td:eq(4)").text();
            gjxh = $("#cttt tr:eq(" + a + ") td:eq(5)").text();
            $("#bbjlx").val(bjlx);
            $("#ggjlx").val(gjlx);
            $("#ggjbh").val(gjbh);
            $("#ggjms").val(gjms);
            ...
            $.ajax({
                ...
                success: function (json) {
                    if (json.success == "success") {
                        ...
                        for (var a = 0; a < len; a++) {
                            $("#h" + hid).append("<li class='parent_li 'id='1" + json.obj[a].section_id + "'><span id='" + json.obj[a].section_id + "' onclick='hbs(this.id)' class='treess' style='font-size:10px;font-family:微软雅黑'><i  id='i" + json.obj[a].section_id + "'class='fa fa-lg fa-plus-circle'>" + json.obj[a].section_name + "</span></li>");
        ...
        if ($("#hs" + hsid).length > 0) {
            ...
            $.ajax({
                ...
                success: function (json) {
                    if (json.success == "success") {
                        var len = json.obj.length;
                        $("#1" + hsid).append("<ul id='hs" + hsid + "'></ul>");
                        for (var a = 0; a < len; a++) {
                            $("#hs" + hsid).append("<li class='treess' ><span class='treess' id='" + json.obj[a].struct_id + "' onclick='query(this.id)' class='treess' style='font-size:10px;font-family:微软雅黑'>" + json.obj[a].struct_id + "</span></li>");
                        }
                    } else {
                        errMessage("没有数据！");
                    }
        ...
        $.ajax({
            ...
            success: function (json) {
                if (json.success == "success") {
                    ...
                    for (var a = 0; a < len; a++) {
                        $("#h" + hid).append("<li class='treess' ><span class='treess' id='" + json.obj[a].struct_id + "' onclick='query(this.id)' style='font-size:10px;font-family:微软雅黑'>" + json.obj[a].struct_id + "</span></li>");
                    }
                } else {
                    if (len1 == "0") {
                        errMessage("没有数据！");
    ...
    function copyAllMem() {
        ...
        $.SmartMessageBox({
            title: "复制提示",
            content: "确认从选中的涵洞中复制所有的跨和构件到当前涵洞吗？该操作会清空当前涵洞所有跨和构件记录！",
            buttons: '[取消][确定]'
        }, function (ButtonPressed) {
            if (ButtonPressed === "确定") {
                showMask();
        ...
        $.ajax({
            ...
            success: function (json) {
                if (json.success == "fail") {
                    ...
                    var data = json.obj;
                    for (var i = 0; i < data.length; i++) {
                        $('#allBridge').append("<option value='" + data[i].culvert_id + "'>" + data[i].culvert_name + "</option>");
                    }
                    $('#allBridge').select2();
                }
            }
