            ...
            buttons: [
                {
                    ...
                    click: function () {
                        var imgName = $('#lookImg h5').html() + ".png";
                        imgName = imgName.replace(/#/g, '-');
                        var imgPath = $('#lookImg span').html();
                        console.log(imgName);
                        window.open(encodeURI("../ImageDownLoadServer?path=" + imgPath + "&imgName=" + imgName));
            ...
            if (s != "" && t != undefined) {
                ...
                if (t.describe.indexOf('宽度') >= 0) {
                    if (data.defect_name.indexOf('裂缝') >= 0) {
                        $(d).typeahead(
                            {
                                source: ['0.10', '0.08', '0.12', '0.06', '0.04', '0.15', '0.14', '0.2'],
                                showHintOnFocus: 'all',
                                matcher: function (g) {
                                    return true;
                                },
...
// 		    var result =/\.[^\.]+/.exec(file_name);
    ...
    function lookImg(d) {
        ...
        $('#lookImg').find('img').eq(0).show();
        $('#lookImg img').eq(1).prop('src', encodeURI("../ImageDownLoadServer?path=" + path + '&' + Math.random()));
        $('#lookImg h5').html($(d).attr('data-memo'));
        $('#lookImg span').html($(d).attr('data-src'));
        $('#lookImg').dialog('open');
    }
    ...
    function format(d) {
        ...
        if (d != undefined) {
            for (var i = 0; i < d.defects.length; i++) {
                ...
                var photo = "<a style='cursor:pointer'  data-memo='" + d.defects[i].defect_photo_memo + "' data-src='" + d.defects[i].defect_photo + "' onclick='lookImg(this)'>查看</a>";
...
<div id="lookImg" hidden="hidden" style="min-width: 500px">
    <h5></h5>
    <div style="position: absolute;transform: translate(-50%, -50%);top: 50%;left: 50% ;">
        <img src="../img/upload.gif" alt="..." style="width:100px" onmousedown="return false;">
    </div>
    <img alt="" src="" style="width: auto;max-height: 400px" onload="$('#lookImg').find('img').eq(0).hide();">
    ...
    <form>
        <fieldset>
            ...
            <div class="form-group">
                <label>部件类型</label>
                <div id="com">
                    <select class="form-control input-sm select2" onchange="comChange()">
            ...
            <div class="form-group">
                <label>构件类型</label>
                <div id="memType">
                    <select class="form-control input-sm select2" onchange="memTypeChange()">
