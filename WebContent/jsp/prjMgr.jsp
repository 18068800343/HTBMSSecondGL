...
<% 
	...
	if(org_id!=null){
		response.sendRedirect("prjMgr2.jsp");
	} 
	%>
<!-- Basic Styles -->
	...
	function stepInit() {
		$('#step1-next').on('click',function() {
			...
			$('#step2').slideDown('normal', 'swing',function() {
				var maintain_org = $("#maintain_org").val();
				initStructTable(chk_type,maintain_org);
					});
...
<body class="">
										...
										<div class="row">
											...
											<div class="col-sm-12 col-md-12">
												<div class="pull-left col-xs-2">
													<select style="width:100%" id="checkSelect" class="form-control input-sm select2"  onchange="reloadData(this)" >
														...
														<div class="row">
															...
															<div class="form-group col-xs-4">
																<label>项目类型</label><i class="text-danger">*</i> <select
																	id="chk_type" class="form-control" style="width: 100%;">
																	<option value="regular">定期检查</option>
																	<option value="special">特殊检查</option>
																</select>
															</div>
															<div class="form-group col-xs-4">
																<label>单位名称</label><i class="text-danger">*</i> <select
																	class="form-control  select2" id="maintain_org" 
																	  style="width: 100%;">
																</select>
															</div>
															<div class="form-group col-xs-4">
																<label>检测单位</label><i class="text-danger">*</i> <input
																	class="form-control" id="ht" value=""
																	placeholder="检测单位" type="text" style="width: 100%;">
															</div>
															<div class="form-group col-xs-4">
															...
															<div class="form-group col-xs-4">
																<label>项目参与人</label><i class="text-danger">*</i> <select
																	id="prj_member" class="form-control select2"
														...
														<thead>
															<tr>
																...
																<th style="width: 100px;"><select
																	class="input-sm form-control" style="width: 100%;"
																	onchange="searchMode(this.value)"><option
																			value="">全部</option>
																		<option>桥梁</option>
																		<option>通道</option>
		...
		<form>
			<fieldset>
				<div class="form-group">
					<label>负责人</label> <select class="form-control input-sm"
						id="manage">

					</select>
				</div>
				<div class="form-group">
					<label>参与人</label> <select class="form-control input-sm select2"
						id="member" multiple>
	...
	<script type="text/javascript">
		...
		var tr;
		var editDel = "<div class='text-align-center'><button class='edit btn btn-warning btn-xs' disabled  data-flag data-hold><span class='glyphicon glyphicon-edit'></span></button>&nbsp;&nbsp;<button class='look btn btn-warning btn-xs'><li class='fa fa-reorder'></li></button>&nbsp;&nbsp;<button class='end btn btn-warning btn-xs' disabled data-hold><span class='fa fa-check'></span></button>&nbsp;&nbsp;<button class='del btn btn-warning btn-xs' disabled><span class='glyphicon glyphicon-trash'></span></button></div>";
				...
				.DataTable(
						{
							...
							"columnDefs" : [
									...
									},
									{
										...
										"searchable" : false,
										"render" : function(data, type, full) {
											var editDell = editDel;
											
											if(full.chk_type=='often'||full.chk_type=='daily'){
												
												editDell =  editDell.replace(
														/data-flag/g,
														'disabled');
		...
		function format(d,o) {
					...
					+ '<td></td>'
					+ '</tr>'
					+ '<tr>' + '<td style="width:150px">公司名称：</td>' + '<td>'
					+ d.maintain_org
					+ '</td>'
					+ '<td>检测单位：</td>'
					+ '<td>'+d.ht_input+'</td>'
		...
		function initPrjTable() {
			...
			$.ajax({
				type : 'POST',
				url : '../PrjMgrServlet?ran='+(new Date()).getTime(),
				dataType : 'json',
				cache:false, 
				data : {
					type : "initPrjTable",
		...
		$('#prjTable').delegate('.add', 'click', function() {
			...
			var chk_type = prjData.chk_type;
			var maintain_org = prjData.zone_id;
			//if(maintain_org!=null&&maintain_org!=""&&maintain_org!=undefined){
				initStructTable(chk_type,maintain_org);
			//}
			addTable.draw(false);
																		...
																		.ajax({
																			...
																			success : function(
																				...
																				if (json.success == "fail") {
																					...
																					prjTable.row(tr).data(data).draw(false);
																					/* prjTable.row(tr).child(format(data,data1)).show(); */
																					$('#editPrj').dialog('close');
																				}
																			}
																		});
															}
		...
		function saveNewPrj() {
			var prj_desc = $('#prj_desc').val();
			var chk_type = $('#chk_type').val();
			var maintain_org = $('#maintain_org option:selected').html();
			var org_id = $('#maintain_org').val();
			var prj_charge_man = $('#prj_charge_man').val();
			var prj_member_list = $('#prj_member').val();
			var ht=$("#ht").val();
		...
		function initStructTable(chk_type,maintain_org) {
			$.ajax({
				type : 'POST',
				url : '../PrjMgrServlet',
				dataType : 'json',
				data : {
					type : "initStructTable",
					chk_type : chk_type,
					maintain_org: maintain_org
				},
				error : function(msg) {
					...
					$('#addTable').dataTable().fnClearTable();
				},
				success : function(json) {
					$('#structTable').dataTable().fnClearTable();
					$('#addTable').dataTable().fnClearTable();
				...
				success : function(json) {
					if (json.success == "fail") {
						...
						for (var i = 0; i < ll.length; i++) {
							d1.append('<option>' + ll[i] + '</option>');
							d2.append('<option>' + ll[i] + '</option>');
							d3.append('<option>' + ll[i] + '</option>');
							d4.append('<option>' + ll[i] + '</option>');
						}
						d1.trigger('change');
						d2.trigger('change');
					}
			...
			$.ajax({
				...
				success : function(json) {
					if (json.success == "fail") {
						...
						var d = $('#maintain_org').empty();
						
						var ll = json.obj;
						for (var i = 0; i < ll.length; i++) {
							d.append('<option value='+ll[i].org_id+' >' + ll[i].org_name + '</option>');
		...
		function releaseAdmin() {//管理员
			...
			editDel = "<div class='text-align-center'><button class='edit btn btn-warning btn-xs' data-hold data-flag><span class='glyphicon glyphicon-edit'></span></button>&nbsp;&nbsp;<button class='add btn btn-warning btn-xs' data-hold data-flag><span class='glyphicon glyphicon-plus'></span></button>&nbsp;&nbsp;<button class='look btn btn-warning btn-xs'><span class='fa fa-reorder'></span></button>&nbsp;&nbsp;<button class='end btn btn-warning btn-xs' data-hold><span class='fa fa-check'></span></button>&nbsp;&nbsp;<button class='del btn btn-warning btn-xs'><span class='glyphicon glyphicon-trash'></span></button></div>";
		}
