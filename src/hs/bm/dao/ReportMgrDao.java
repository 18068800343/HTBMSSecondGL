...
public class ReportMgrDao {
	...
	public String buildPackageNew(String id, String prj_id, String mode,String package_name) {
		...
		MyDataOperation dataOperation = new MyDataOperation(MyDataSource.getInstance().getConnection());
		String sql = " SELECT	DISTINCT cbp.photo_path,cbp.defect_serial FROM	chk_brg_photo cbp LEFT JOIN chk_brg_defect cbdt ON cbp.defect_serial = cbdt.defect_serial LEFT JOIN chk_brg_record cbr on cbdt.mbr_chk_id = cbr.mbr_chk_id LEFT JOIN chk_span_record csr on cbr.span_chk_id = csr.span_chk_id LEFT JOIN chk_brg_regular cbrr on cbrr.chk_id = csr.chk_id LEFT JOIN chk_project_info cpi on cpi.prj_id = cbrr.prj_id WHERE cpi.prj_id = ? ";
		try {
