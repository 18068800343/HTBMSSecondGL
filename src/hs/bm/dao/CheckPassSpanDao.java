...
public class CheckPassSpanDao {

	private static CheckPassSpanDao checkPassSpanDao;

	private static final Log log = LogFactory.getLog(CheckPassSpanDao.class);

	public static CheckPassSpanDao getInstance() {
		if (checkPassSpanDao == null) {
			checkPassSpanDao = new CheckPassSpanDao();
		}
		return checkPassSpanDao;
	}

	public List<BridgeMemAll> initComBox(String brgid, String direction, String span_no) {
		String sql = "select b2.r_id,b2.member_no,b2.member_type,b2.direction,b2.span_no,d2.component9,d2.member_id,d2.distr_name from dic_brg_struct_member_def as d2 INNER JOIN "
				+ "(select b1.r_id,b1.s_id,b1.member_type,b1.member_no,d1.direction,d1.span_no from pass_mbr_info as b1,"
				+ "(SELECT s_id,pass_id,direction,span_no,pass_type_id from pass_span_info where pass_id=? and direction=? and span_no=?) as d1 "
		...
		try {
			while (rs.next()) {
				...
				ba.setComponent9(rs.getString("component9"));
	...
	public List<SpanMem> initTable(String span_chk_id) {
		...
		try {
			while (rs.next()) {
				...
				sm.setBridge_id(rs.getString("pass_id"));
				...
				sm.setComponent_name(rs.getString("component9"));
	...
	public List<ChkBrgDefect> getDefectList(String mbr_chk_id) {
		String sql = "select * from chk_pass_defect where mbr_chk_id=?";
		MyDataOperation dataOperation = new MyDataOperation(MyDataSource.getInstance().getConnection(), false);
		...
		try {
			while (rs.next()) {
				...
				String sql2 = "select * from chk_pass_photo where defect_serial=?";
	...
	public int delMem(String mbr_chk_id) {
		String sql = "delete from chk_pass_record where mbr_chk_id=?";
		MyDataOperation dataOperation = new MyDataOperation(MyDataSource.getInstance().getConnection());
	...
	public List<DicDefect> getDefect(String component_name) {
		...
		MyDataOperation dataOperation = new MyDataOperation(MyDataSource.getInstance().getConnection());
		String sql = "select defect_f_id from dic_mbr_defect_def where component_id = (select component_id from dic_brg_struct_component_def where component_name=? and specification='通道检查记录') order by ratio";
	...
	public List<DicDefect> getDefect(String component_name, String defect_name, String defect_name_f) {
		...
		String sql = "select IFNULL(a.defect_id,'') as defect_id,IFNULL(a.defect_name,'') as defect_name,IFNULL(a.defect_loc_def,'') as defect_loc_def,IFNULL(a.defect_def,'') as defect_def,IFNULL(a.defect_template,'') as defect_template,IFNULL(a.defect_attr,'') as defect_attr,b.* from dic_mbr_defect_s as a,dic_mbr_defect_f as b where a.defect_f_id=b.defect_f_id and a.defect_name=? and b.defect_f_name=? and a.defect_f_id in (select defect_f_id from dic_mbr_defect_def where component_id = (select component_id from dic_brg_struct_component_def where component_name=? and specification='通道检查记录'))";
		MyDataOperation dataOperation = new MyDataOperation(MyDataSource.getInstance().getConnection());
	...
	public int editDefect(ChkBrgDefect cbd, String current) {
		...
		MyDataOperation dataOperation = new MyDataOperation(MyDataSource.getInstance().getConnection());
		int i = dataOperation.executeUpdate(sql,
				new String[] {cbd.getDefect_name_f(),cbd.getDefect_name(), cbd.getDefect_location_desc(), cbd.getDefect_count(), 
						 cbd.getDefect_attr(), cbd.getDefect_location_desc_val(),
	...
	public int delDefect(String id) {
		String sql = "delete from chk_pass_defect where defect_serial=?";
		MyDataOperation dataOperation = new MyDataOperation(MyDataSource.getInstance().getConnection());
	...
	public int changeRepState(String id, String state) {
		String sql = "update  chk_pass_defect set repair_state=?,current=? where defect_serial=?";
		MyDataOperation dataOperation = new MyDataOperation(MyDataSource.getInstance().getConnection());
	...
	public int changeDevState(String id, String state) {
		String sql = "update  chk_pass_defect set develop_state=?,current=? where defect_serial=?";
		MyDataOperation dataOperation = new MyDataOperation(MyDataSource.getInstance().getConnection());
		int i = dataOperation.executeUpdate(sql, new String[] { state, "1", id });
		dataOperation.close();
		return i;
	...
	public int overCheck(String span_chk_id, OperationConstruct oc) {
		MyDataOperation dataOperation = new MyDataOperation(MyDataSource.getInstance().getConnection());
		String sql = "update  chk_pass_span_record set chk_state='1' where span_chk_id=?";
	...
	public int saveDefectCount(List<DefectCount> ll, String defect_serial) {
		String sql = "insert into defect_count_pass (defect_serial,struct_id,chk_id,member_no,defect_id,defect_record,defect_record_val,defect_record_type,save_date) values (?,?,?,?,?,?,?,?,?)";
		MyDataOperation dataOperation = new MyDataOperation(MyDataSource.getInstance().getConnection(), false);
		int i = 0;
	...
	public int editDefectCount(List<DefectCount> ll, String defect_serial) {
		String sql = "delete from defect_count_pass where defect_serial=?";
		MyDataOperation dataOperation = new MyDataOperation(MyDataSource.getInstance().getConnection(), false);
		int i = 0;

		i = dataOperation.executeUpdate(sql, new String[] { defect_serial });
		if (i > 0) {
			sql = "insert into defect_count_pass (defect_serial,struct_id,chk_id,member_no,defect_id,defect_record,defect_record_val,defect_record_type,save_date) values (?,?,?,?,?,?,?,?,?)";
			for (DefectCount dc : ll) {
				i = dataOperation.executeUpdate(sql, new Object[] { defect_serial, dc.getStruct_id(), dc.getChk_id(),
						dc.getMember_no(), dc.getDefect_id(), dc.getDefect_record(), dc.getDefect_record_val(),
						dc.getDefect_record_type(), new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()) });
	...
	public boolean span_no_last(String id, String span_no, String direction) {
		boolean flag = false;
		String sql = "select * from pass_span_info where pass_id=? and direction=? ORDER BY span_no DESC LIMIT 0,1";
		MyDataOperation dataOperation = new MyDataOperation(MyDataSource.getInstance().getConnection());
	...
	public int changeChkState(String span_chk_id, String chk_id, String prj_id, String audit_state) {
		...
		sql = "update chk_pass_span_record set chk_state='0',pdf=? where span_chk_id=?";
	...
	public List<String> reference(String id, String span_no, String direction) {
		...
		String sql = "select * from pass_mbr_info where s_id=(select s_id from pass_span_info where pass_id=? and direction=? and span_no=?) and member_no like '%墙身%'";
		MyDataOperation dataOperation = new MyDataOperation(MyDataSource.getInstance().getConnection(), false);
	...
	public int deletePhotoById(String photo_id) {
		int i = 0;
		MyDataOperation mdo = new MyDataOperation(MyDataSource.getInstance().getConnection(), false);
		String sql = " delete from chk_pass_photo where photo_id=?";
	...
	public int updatePhotoNameById(String photo_id, String photo_name) {
		int i = 0;
		MyDataOperation mdo = new MyDataOperation(MyDataSource.getInstance().getConnection(), false);
		String sql = " UPDATE chk_pass_photo SET photo_name=? WHERE photo_id=?";
	...
	public List<ChkPassPhoto> selectPhotoBySerial(String photo_serial) {
		...
		try {
			while (rs.next()) {
				...
				ChkPassPhoto cpp = new ChkPassPhoto();
				cpp.setPhoto_id(rs.getString("photo_id"));
				cpp.setPhoto_name(rs.getString("photo_name"));
				cpp.setPhoto_date(rs.getString("photo_date"));
				cpp.setPhoto_path(rs.getString("photo_path"));
				cpp.setDefect_serial(rs.getString("defect_serial"));
				photoList.add(cpp);
			}
		} catch (SQLException e) {
