...
public class CheckCulvertDao {

private static CheckCulvertDao checkCulvertDao;
	
	public static CheckCulvertDao getInstance(){
		if(checkCulvertDao==null){
			checkCulvertDao=new CheckCulvertDao();
		}
		return checkCulvertDao;
	}
	...
	public List<CulSpanInfo> getSpan(String culvert_id){
		String sql = "select * from cul_span_info where culvert_id=? ORDER BY span_no";
		MyDataOperation dataOperation = new MyDataOperation(MyDataSource.getInstance().getConnection());
		ResultSet rs = dataOperation.executeQuery(sql, new String[]{culvert_id});
		List<CulSpanInfo> lb=new ArrayList<CulSpanInfo>();
		try {
			while (rs.next()) {
				CulSpanInfo dmd = new CulSpanInfo();
				dmd.setS_id(rs.getString("s_id"));
				dmd.setCulvert_id(rs.getString("culvert_id"));
				dmd.setDirection(rs.getString("direction"));
				dmd.setSpan_no(rs.getInt("span_no"));
				dmd.setCul_type_id(rs.getString("cul_type_id"));
				lb.add(dmd);
			}
		} catch (SQLException e) {
			e.printStackTrace();
	 ...
	 public ResObj CheckState(String username,OperationConstruct oc){
	    	...
	    	String sql="select a.*,b.* from (select * from chk_project_info where prj_id=? and (prj_charge_man like '%$%' or prj_member like '%$%')) as a RIGHT JOIN "
	    			+ "(select * from chk_culvert_regular where chk_id=?) as b on 1=1;";
	 ...
	 public BridgeChk getAllSpansData(BridgeChk bc){
		 String sql = "select * from chk_culvert_span_record where chk_id=?";
			MyDataOperation dataOperation = new MyDataOperation(MyDataSource.getInstance().getConnection());
	 ...
	 public int addSpan(ChkSpanRecord cs){
		 String sql = "insert into chk_culvert_span_record(span_chk_id,chk_id,direction,span_no,span_memo,span_suggestion,chk_time,chk_weather,temp,chk_state,record_person,recheck_person) values(?,?,?,?,?,?,?,?,?,?,?,?)";
			MyDataOperation dataOperation = new MyDataOperation(MyDataSource.getInstance().getConnection(),false);
	 ...
	 public int editSpan(ChkSpanRecord cs){
		 String sql = "update chk_culvert_span_record set span_memo=?,span_suggestion=?,chk_time=?,chk_weather=?,temp=?,chk_state=?,recheck_person=?,pdf=? where span_chk_id=?";
			MyDataOperation dataOperation = new MyDataOperation(MyDataSource.getInstance().getConnection(),false);
			//String uuid=UUID.randomUUID().toString().replaceAll("-", "");
			int i = dataOperation.executeUpdate(sql, new String[]{
					cs.getSpan_memo(),
	 ...
	 public int delSpan(String span_chk_id){
		 String sql = "delete from chk_culvert_span_record where span_chk_id=?";
			MyDataOperation dataOperation = new MyDataOperation(MyDataSource.getInstance().getConnection(),false);
			//String uuid=UUID.randomUUID().toString().replaceAll("-", "");
			int i = dataOperation.executeUpdate(sql, new String[]{
					span_chk_id
	 ...
	 public int overCheck(OperationConstruct oc,String chk_id, String eval_level, String inspection_person){
			...
			MyDataOperation dataOperation = new MyDataOperation(MyDataSource.getInstance().getConnection());
			String sql = "UPDATE chk_culvert_regular set check_date=CURDATE(),audit_state='1',eval_level=?,inspection_person=? where chk_id=?";
			int i = dataOperation.executeUpdate(sql, new String[]{
					eval_level,
					inspection_person,
					chk_id
	 ...
	 public List<DefectCount> getDefectCount(String defect_serial){
		 String sql = "select * from defect_count_culvert where defect_serial=?";
		 MyDataOperation dataOperation = new MyDataOperation(MyDataSource.getInstance().getConnection());
	...
	public Map<String, String> initPhoto(String id, String prj_id){
		...
		String sql = "select * from cul_prj_photo where culvert_id=? and prj_id=?";
		MyDataOperation dataOperation = new MyDataOperation(MyDataSource.getInstance().getConnection());
	...
	public void updatePhoto(String id, String prj_id, String path, String photo_type) {
		String sql = "update cul_prj_photo set path=? where culvert_id=? and prj_id=? and photo_type=?";
		MyDataOperation dataOperation = new MyDataOperation(MyDataSource.getInstance().getConnection());
		int i = dataOperation.executeUpdate(sql, new String[]{path, id, prj_id, photo_type});
		if(i<=0){
			sql = "insert into cul_prj_photo(culvert_id, prj_id, path, photo_type) values(?,?,?,?)";
