...
public class AutoSendMessageHealth implements Runnable{
	
	
	private String message = "";
	
	public AutoSendMessageHealth() {
		super();
	}
	...
	public void run() {
		...
		brgSystems_w=GetFileSizeDao.getInstance().getBrgSystems("s");//健康监测
		for(BrgSystem brgSystem_w:brgSystems_w){
			...
			if(brgCardAdminIds.size()>0){
				...
				if("淮安大桥".equals(brg_name)){
					System.out.println(brg_name);
				}
				try {
			...
			List<BrgHealthStatistic> brgWeightStatistics = GetFileSizeDao.getInstance().GetHealthData(brgSystem_w.getBridge_id());
			String end_time="";
			...
			if(!"".equals(brg_name)&&!"".equals(end_time)){
				try {
					...
					if(day>=7&&("3day".equals(out_time)||"0hour".equals(out_time))){
						try {
							if(isInDate){
							String code = ShortMessageServiceV2.getInstance().SendMessage(phone_no,"{system:'"+brg_name+"健康监测系统',time:'"+day+"天'}","SMS_105180086");
							}else{
						     ...
						     waitSendMessage.setMessage_variable("{system:'"+brg_name+"健康监测系统',time:'"+day+"天'}");
							...
							GetFileSizeDao.getInstance().updateBrgSystemOutTime(brgSystem_w.getBridge_id(), "7day","s");
						...
						try {
							if(isInDate){
								...
								GetFileSizeDao.getInstance().updateBrgSystemOutTime(brgSystem_w.getBridge_id(), "3day","s");
						} catch (ClientException e) {
							e.printStackTrace();
						}
						...
						try {
							if(isInDate){
								String code = ShortMessageServiceV2.getInstance().SendMessage(phone_no,"{system:'"+brg_name+"健康监测系统',time:'"+hour+"小时'}","SMS_105180086");
								}else{
							     WaitSendMessage waitSendMessage = new WaitSendMessage();
							     waitSendMessage.setBridge_id(brgSystem_w.getBridge_id());
							     waitSendMessage.setCode("SMS_105180086");
							     waitSendMessage.setIf_send("0");
							     waitSendMessage.setMessage_variable("{system:'"+brg_name+"健康监测系统',time:'"+hour+"小时'}");
							     waitSendMessage.setPhone_nums(phone_no);
							     waitSendMessage.setWait_send_time(ReadFileUtil.getTomorrow(date));
							     SendMessageDao.getInstance().insertWaitSendMessage(waitSendMessage);
								}
								GetFileSizeDao.getInstance().updateBrgSystemOutTime(brgSystem_w.getBridge_id(), "1hour","s");
						} catch (ClientException e) {
							e.printStackTrace();
						}
					}else{
						if(day==0&&hour==0&&minute<=60){
							GetFileSizeDao.getInstance().updateBrgSystemOutTime(brgSystem_w.getBridge_id(), "0hour","s");	
						}
					}
	...
	public static void main(String[] args){
		Thread thread = new Thread(new AutoSendMessageHealth());
		thread.start();
	}
}
